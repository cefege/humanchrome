...
### IMP-0155 · Matrix runner regression — 7/16 tests time out after IMP-0113/0118/0137 actionability changes (bug) · score: 7

- **Proposed by**: claude-loop · 2026-05-19 (matrix evidence from IMP-0139 verification)
- **Status**: proposed
- **Why**: After IMP-0139 unblocked `pnpm e2e:isolated` locally, the matrix run surfaces 7/16 failures that all share the same shape: `{"isError":true,"transportError":"The operation was aborted due to timeout"}` at the 15s HTTP timeout — NOT the expected actionability failure envelope (e.g. `not_visible`, `unstable_bbox`, `occluded_by:*`, `disabled`, `not_editable`). The 2026-05-17 all-green baseline (`docs/e2e-runs/2026-05-17_all-green.json`) had 16/16 PASS. Regression introduced sometime between 2026-05-17 and 2026-05-19; suspected commits (in landing order): IMP-0118 (time-window checkStable), IMP-0113 (offscreen scroll-into-view + tighter stability), IMP-0137 (actionability hard-fail when not loaded), IMP-0136 (inject-script bridge classify), IMP-0135 (wait-for race), IMP-0138 (wait-helper TDZ). The actionability runtime must be hanging inside the polling loop instead of returning a structured failure — perhaps `awaitActionable`'s deadline isn't honored when `checkVisible`'s scroll-recovery branch triggers, or `hasActiveAnimation` enters an infinite resample loop. All seven failing rows hit `chrome_click_element` or `chrome_fill_or_select` — i.e. tools that go through `awaitActionable`. The locator_handler test fails for a different reason (timeout on a 'list' action — likely a separate hang in locator-handler.ts that doesn't touch actionability).
- **Cost**: M (bisect 6 commits, fix the loop)
- **Value**: L (matrix evidence is the load-bearing E2E gate per CLAUDE.md; 9/16 PASS is unacceptable — must be 16/16 before any subsequent chrome-extension PR can claim local verification)
- **Repro**: `pnpm e2e:isolated` from a clean main. Failing rows: `IMP-0097 offscreen (left:-9999) fails not_visible`, `IMP-0097 animation unstable_bbox`, `IMP-0097 occluded_by:*`, `IMP-0097 disabled button`, `IMP-0097 aria-disabled button`, `IMP-0097 readonly input rejects fill`, `IMP-0101 locator_handler list returns ok`. All return `transportError:"The operation was aborted due to timeout"` at 15000ms. Full evidence in `/tmp/e2e-result.json` after the run.
- **Fix sketch**: (1) `git bisect` between `docs/e2e-runs/2026-05-17_all-green.json`'s commit and `main` (8 candidate commits IMP-0118..IMP-0138) to find the first commit that regresses any of the 7 failing rows. (2) Likely culprit: `app/chrome-extension/inject-scripts/actionability.js:87` — the `while (true)` poll inside `awaitActionable` reads `Date.now() >= deadline` on every iteration, but the `scrollIntoView` recovery branch + the `checkStable` resample don't share that deadline. If `hasActiveAnimation` returns true on every cycle (e.g. for the `#sliding-btn` 4s infinite-translate animation), the `checkStable` Promise never resolves to `null` and the outer loop never terminates because the inner Promise blocks the loop forever. (3) Confirm by adding an `AbortSignal` or explicit elapsed-time check around `checkStable`'s rAF chain — if `Date.now() - startedAt > timeoutMs`, return `unstable_bbox` immediately. Apply the same fix to `isOffscreenButPresent` recovery (one-shot scroll already respects deadline via the outer while). (4) Re-run `pnpm e2e:isolated` — expect 16/16 PASS to match the 2026-05-17 baseline.
- **Notes**: Blocking IMP-0139's "harness fully verified" status. The bridge handshake bug from IMP-0139 IS fixed (proven by `discovered bridge instance=... (handshake 2651ms)` in every run), but the test rows downstream of the harness are themselves broken. Filing this as a separate IMP rather than reopening IMP-0139 because the root cause (actionability runtime) is in a different file from the harness (matrix runner).
