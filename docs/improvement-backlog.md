...
- **Summary**: New `HistoryDeleteTool` (`tools/browser/history-delete.ts`, 165 lines) wired into TOOL_NAMES/TOOL_SCHEMAS/TOOL_CATEGORIES. Params: `url` (deleteUrl), `startTime`/`endTime` (deleteRange), `all: true` (deleteAll). Mutually exclusive selectors enforced; `all` requires strict `true`; range start>end rejected. Lifted `parseDateString` to a shared `parseHistoryDateString` helper so both history tools share date semantics. 11 new tests (705 extension total, 69 bridge), build green.
- **Worktree**: /Users/mike/Documents/Code/humanchrome/.claude/worktrees/agent-a05d955e2c8b7b832 · branch worktree-agent-a05d955e2c8b7b832
