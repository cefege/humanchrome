...
- **Status**: in-progress (slices 1 + 2 landed: model-registry + memory-pool extracted under `utils/semantic-similarity/`)
...
- **Proposed by**: optimization-scout · 2026-05-06
- **Status**: in-progress (slices 1-5: navigate.ts, scroll.ts, wait.ts, assert.ts, click.ts, fill.ts typed; ~83 casts removed; click.ts also dedup'd via runClickStep helper, fill.ts via logFallback helper)
...
- **Proposed by**: optimization-scout · 2026-05-08
- **Status**: in-progress (slices 1-4: inheritance, shorthand, sheet-inspector, specificity-parser all extracted under `cssom/`; orchestrator shrunk from 1552 LoC to ~860 LoC)
