...
- **Status**: proposed
- **Why**: Both `click-helper.js:340-343` and `fill-helper.js:399-405` define `runActionability` that checks `window.__actionability`. When the actionability primitive isn`t loaded (because `actionability.js`failed to inject for ANY reason — file missing from build, CSP blocked, race with cleanup, etc.), the helper logs a`console.warn`and returns`Promise.resolve({ok: true})`. Effect: ALL pre-action checks (visible/stable/enabled/editable/hit-test) are silently skipped, exactly as if `force:true` were set everywhere. The user thinks IMP-0097 actionability is protecting them; in reality it isn`t, until they happen to check DevTools console. Symptom in production: regression to pre-IMP-0097 silent-click-on-overlay behavior, no error envelope, no failure token. Comment says "production callers always inject it alongside fill-helper" — but that`s a hope, not a guarantee. If `interaction.ts`ever drops`actionability.js`from the`injectContentScript` list (regression during refactor), it silently disables protection across all click/fill paths.
- **Cost**: S
- **Value**: L
- **Repro**: Mock `window.__actionability` undefined in a vitest test where the click-helper shim runs against a div with `pointer-events:none` overlay. The shim returns `{ok:true}` and dispatches a click; should return NOT_ACTIONABLE. Or: temporarily delete `actionability.js` from build output then call `chrome_click_element` against any page — clicks proceed even on hidden elements.\n- **Fix sketch**: `app/chrome-extension/inject-scripts/click-helper.js:338-345` and `fill-helper.js:399-406` — replace permissive fallback with a hard-fail returning `{ok:false, failures:["actionability_unavailable"]}`. Background tool then surfaces NOT_ACTIONABLE with a clear `actionability_unavailable` failure token so the agent can branch. Add a new ToolErrorCode subclass or extend the existing failure-token enum so the contract is explicit. Alternative: detect missing helper at the dispatcher (`interaction.ts` injectContentScript list) and reject before sending the message — failing closer to the build-time configuration error.\n- **Notes**: Pairs with adding a self-test in `interaction.ts` that verifies `window.__actionability` exists after injection completes — catches the build-misconfiguration class of bug at the contract boundary. Bonus: a `chrome_runtime_info` extension to report which helpers loaded per tab would make diagnosis trivial.
