...
- **Status**: done (2026-05-17; primary/relay daemon split lives in `app/native-server/src/bridge-orchestrator.ts` + `src/util/daemon-socket.ts`. First bridge that spawns acquires a UDS lock and owns :12306 + the NM router. Subsequent bridges spawned by SW reloads fail the lock, become byte-pipe relays from their own stdin → existing daemon UDS, and exit on stdin close. The daemon's NM source hot-swaps between own-stdin and the most-recent relay socket via `NativeMessagingHost.setStreams`, so MCP/SSE clients on :12306 never see TCP RST. Requests landing in the brief no-source window fail fast with `Bridge has no active native-messaging source` instead of hanging 120s. Matrix runner isolates its bridge from the user's main daemon via `HC_INSTANCE_REGISTRY_DIR` + `HC_BRIDGE_DAEMON_SOCKET` env overrides. Set `HC_DISABLE_DAEMON=1` for legacy standalone behaviour. Coverage: `src/util/daemon-socket.test.ts` (5 tests — acquire/stale/relay round-trip) and `src/bridge-orchestrator.test.ts` (2 tests — full primary→source-close→relay-handoff regression).)
