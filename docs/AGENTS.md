...
| `INJECTION_FAILED`      | Chrome refused to inject a content script (restricted URL like `chrome://`, devtools://, store pages). For `chrome_inject_script` also covers per-frame execution failures — `details.reason` is `CSP_BLOCKED` when the page's CSP refused the script (LinkedIn `strict-dynamic`, Gmail `unsafe-eval`) or `INJECTION_ERROR` for any other runtime throw inside the injected function.                                                                                                                                                                                                                                                                         | Don't retry the same URL. On `CSP_BLOCKED`, fall back to `chrome_javascript` (CDP path bypasses page CSP) or `chrome_inject_script` with `type:'ISOLATED'`. Otherwise tell the user the page can't be automated.                                                                                                                                                               |
| `INJECTION_TIMEOUT`     | Injection didn't settle inside the bridge's internal budget — content script didn't respond to ping, OR (for `chrome_inject_script`) `chrome.scripting.executeScript` hung past 5s. `details.phase` names which stage failed (`bridge inject`, `MAIN-world inject`, `MAIN-world verify`).                                                                                                                                                                                                                                                                                                                                                                     | Retry once; if persistent, the page is likely hostile to automation (`details.phase: "bridge inject"` against freelancermap.de is the canonical case — fall back to `chrome_javascript`).                                                                                                                                                                                      |
...
| `NOT_ACTIONABLE`        | Target exists but failed pre-action checks (visible/stable/enabled/editable/hit-test). `details.failures` names the failed checks (`['not_visible', 'occluded_by:#cookie-banner', 'disabled', 'unstable_bbox', 'not_editable']`). IMP-0137: `'actionability_unavailable'` token means the actionability primitive (`window.__actionability`) wasn't injected — the click/fill helper hard-failed rather than silently skipping the suite. If you see this consistently, the extension build is broken (`actionability.js` missing from `inject-scripts/`); the dispatcher's `assertHelperPresent` guard normally surfaces this earlier as `INJECTION_FAILED`. | Wait for the offending overlay (`chrome_await_element` on the blocker, `chrome_wait_for` on text), scroll, or dismiss the overlay; then retry. Pass `force: true` on the action to skip the suite (`scrollIntoView` still runs). For `actionability_unavailable`: rebuild/reinstall the extension; `force: true` is a temporary workaround but disables ALL pre-action checks. |
