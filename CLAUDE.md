...
- **`pnpm test` should be green.** If you see a `preHandler.test.ts` timeout, the two parallel-jest flakes (`POST with loopback Host`, `POST with chrome-extension:// Origin`) are quarantined under IMP-0123 — un-skipping requires fixing the root cause (split into separate test files, or `beforeAll(buildServer)` + `afterAll(close)` instead of per-test recreation). Any other test failure is real; don't dismiss it as flake.
