...
> **Authoring is unchanged after IMP-0177 (single-tool MCP dispatcher).** The bridge serves tools in two modes — `lazy` (the default since IMP-0185; one `humanchrome(name, args)` dispatcher whose description carries the catalog, ~10.86× boot-manifest reduction) and `legacy` (full 96-tool MCP manifest, opt in via `HUMANCHROME_TOOL_MODE=legacy` for clients that need the old shape during a migration). Either mode reads `TOOL_SCHEMAS` directly — append your tool there per step 2 and it auto-appears in both surfaces. Do not edit `packages/shared/src/tool-index.ts` by hand; it builds from `TOOL_SCHEMAS`.
...
- **Dispatcher description is byte-stable (IMP-0181).** The IMP-0177 dispatcher's description blob is pinned to `packages/shared/src/tool-index.snapshot.json`. Any drift fails the contract test `tool-index.snapshot.test.ts`. The blob feeds Anthropic's prompt cache (5-min TTL); a single byte change invalidates the cache for every connected client on every restart. Disciplines: tools are sorted by name in the description (already enforced by `buildDispatcherDescription`); no timestamps / env vars / hostnames / version strings inside any tool description; intentional changes regenerate the snapshot via `node packages/shared/scripts/regen-tool-index-snapshot.mjs` (or `UPDATE_SNAPSHOT=1 npx vitest run src/tool-index.snapshot.test.ts`). Banned-patterns assertions in the contract test catch the silent failure mode where a dynamic string slips into a tool description.
