...
(() => {
  ...
  const handleExecution = (event) => {
    ...
    window.dispatchEvent(
      new CustomEvent('humanchrome:response', { detail: { requestId, data, error } }),
    );
  };
  const initialize = () => {
    window.addEventListener('humanchrome:execute', handleExecution);
    window.addEventListener('humanchrome:cleanup', cleanup);
    window[SCRIPT_ID] = true;
  };
  const cleanup = () => {
    window.removeEventListener('humanchrome:execute', handleExecution);
    window.removeEventListener('humanchrome:cleanup', cleanup);
