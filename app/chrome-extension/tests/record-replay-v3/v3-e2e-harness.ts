...
export interface V3E2EHarness {
  ...
  /** Dispose the harness, releasing resources */
 ...
 * @description Assembles a complete V3 runtime for integration testing.
 */
export function createV3E2EHarness(options: V3E2EHarnessOptions = {}): V3E2EHarness {
  ...
  // 6) RunExecutor - bridges scheduler and runner
...
/**
 * Create the RunExecutor used by E2E tests.
 */
function createE2EExecutor(deps: {
  ...
  return async (item: RunQueueItem): Promise<void> => {
    ...
    // Sync attempt/tabId from queue item back to the RunRecord.
