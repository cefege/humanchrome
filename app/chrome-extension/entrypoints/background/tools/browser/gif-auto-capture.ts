...
// Gif auto-capture state is per-tab metadata; all entries route to the
// system bucket. The previous implementation had no tab-close listener
// at all, so closed tabs leaked TabCaptureState until clearAllAutoCapture
// ran.
const tabStates = createOwnedRegistry<TabCaptureState>();
...
export async function startAutoCapture(
  ...
  if (tabStates.has(undefined, tabId)) {
  ...
  try {
    ...
    tabStates.set(undefined, tabId, state);
...
export async function stopAutoCapture(tabId: number): Promise<{
  ...
  const state = tabStates.get(undefined, tabId);
  if (!state) {
  ...
  try {
    ...
    // Cleanup
    tabStates.delete(undefined, tabId);
    try {
      await cdpSessionManager.detach(tabId, CDP_SESSION_KEY);
    } catch {
      // Ignore
...
export function isAutoCaptureActive(tabId: number): boolean {
  return tabStates.has(undefined, tabId);
}
...
export async function captureFrameOnAction(
  ...
  const currentState = tabStates.get(undefined, tabId);
  if (!currentState) {
    return { success: true };
  }
  ...
  const capturePromise = (async () => {
    ...
    for (let i = 0; i < plan.frames; i++) {
      const activeState = tabStates.get(undefined, tabId);
...
export async function clearAllAutoCapture(): Promise<void> {
  const tabIds = [...tabStates.entries()].map((e) => e.tabId);
