...
export function useBuilderStore(initial?: FlowV2 | null) {
  ...
  function setNodePosition(id: string, x: number, y: number) {
    ...
    // Skip the undo stack — only user-triggered operations (connect/add/delete) record history.
  }
  ...
  function onConnect(sourceId: string, targetId: string, label: string = 'default') {
    ...
    // Single outgoing edge per label: drop existing edges with the same source + label
    for (let i = edges.length - 1; i >= 0; i--) {
      const e = edges[i];
  ...
  // Fallback layout: layered + barycenter ordering (no external dep)
  ...
  // Auto layout (prefers ELK): dynamic import of elkjs to avoid resident bundle weight,
