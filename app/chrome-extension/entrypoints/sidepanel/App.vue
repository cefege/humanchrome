          ...
          <div class="em-stats-bar">
            <span class="em-stats-text">
              <template v-if="markerSearch">
                Showing <strong>{{ filteredMarkers.length }}</strong> annotations (of
                {{ markers.length }} , {{ groupedMarkers.length }} hosts)
              </template>
              <template v-else>
                Total <strong>{{ markers.length }}</strong> annotations,
                <strong>{{ groupedMarkers.length }}</strong> hosts
              </template>
            </span>
          </div>
          ...
          >
            ...
            >
              <div class="em-domain-info">
                ...
                <span class="em-domain-count">{{ domainGroup.count }} annotations</span>
              </div>
            </div>
...
async function deleteMarker(marker: ElementMarker) {
  try {
    const confirmed = confirm(`Delete annotation "${marker.name}" ?`);
    if (!confirmed) return;
